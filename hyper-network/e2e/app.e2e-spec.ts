...
describe('Starting tests for hyper-network', function() {
  ...
  it('website title should be hyper-network', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('hyper-network');
    })
  });

  it('network-name should be network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('network@0.0.1.bna');
    });
  });

  it('navbar-brand should be hyper-network',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hyper-network');
    });
  });
